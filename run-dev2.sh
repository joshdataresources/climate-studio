...
# --- 1) Node backend on :3003 (USGS streamflow, NOAA tiles, proxies EE tiles to :5001) ---
echo "▶ starting Node backend on :3003…"
( cd backend && [ -d node_modules ] || npm install >/dev/null 2>&1; \
  CLIMATE_SERVICE_URL="http://localhost:5001" PORT=3003 node server.js ) &
PIDS+=($!)
...
( for i in $(seq 1 90); do
    curl -fsS -o /dev/null --max-time 1 http://localhost:8082 && { sleep 1; open "http://localhost:8082" 2>/dev/null; break; }
    sleep 1
  done ) &

# --- 4) frontend (foreground; Ctrl-C stops everything) ---
echo "▶ starting frontend on :8080  (Ctrl-C to stop all)…"
cd apps/climate-studio && npx vite --host 127.0.0.1 --port 8082 --force
