...
const SVG_PATHS = {
  ...
  factories: "M12 3L2 12H5V20H10V14H14V20H19V12H22L12 3ZM17 18H16V12H8V18H7V10.19L12 5.69L17 10.19V18Z",
      ...
      <div className="flex flex-col h-full isolate items-center p-[8px] relative justify-between">
        ...
        <div className="flex flex-col gap-[8px] items-center relative shrink-0">
          ...
          {/* <button
            ...
            aria-pressed={activeMenuItem === 'settings'}
            title="Settings"
          >
            <div className="relative shrink-0 size-[24px]">
              <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
                <path
                  d={SVG_PATHS.settings}
                  fill={activeMenuItem === 'settings' ? '#5A7CEC' : (isDark ? '#D1D5DB' : '#697487')}
                />
              </svg>
            </div>
            <p
              className={`font-['Inter',sans-serif] font-semibold leading-[normal] not-italic relative shrink-0 text-[10px] text-center w-[50px] ${
                activeMenuItem === 'settings' ? 'text-[#5a7cec]' : (isDark ? 'text-white' : 'text-black')
              }`}
            >
              Settings
            </p>
